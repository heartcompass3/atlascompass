...
app.post("/api/gemini/generate", async (req, res) => {
  try {
    ...
    const platformInstruction: Record<string, string> = {
      tiktok: "סרטון טיקטוק מהיר וקצבי (כ-25 שניות). פתח עם הוק (Hook) חזק תוך 3 שניות, דבר בגובה העיניים, בלי הקדמות, קצר, קולע וממוקד בשינוי תודעתי מהיר.",
      reels: "סרטון רילס (כ-60 שניות). שלב סיפור קצר או דוגמה חיה (למשל: מקרה מהקליניקה או סיפור אישי), הסבר את המנגנון מאחוריו, וסיים בתובנה מעשית.",
      shorts: "סרטון שורטס מהודק ומדויק. משפטים קצרים, קצב מהיר, התמקדות בפרדוקס או ניפוץ מיתוס לגבי המנגנון.",
      youtube_long: "סרטון יוטיוב ארוך ומעמיק. הרחב בצורה מפורטת על המנגנון, הבא דוגמאות עומק, ניתוח פסיכולוגי ותהליך עבודה קליני מעורר מחשבה."
    };
    ...
    let coreTaskText = "";
    if (readyText) {
      coreTaskText = `ערוך ושפר את התסריט הבא כך שיהיה נקי, מדויק ומוכן לקריאה בפרומפטר:\n"${readyText}"`;
    } else {
      coreTaskText = `
...
- טון הדיבור: שיח בגובה העיניים, כנה, לא מתנשא, שמבין את עולמם הפנימי והתרבותי. שימוש בדוגמאות מעולם המתבגרים (מסכים, גיימינג, חרדת דחייה חברתית בקבוצת השווים, לחץ לימודי, צורך באוטונומיה, הסתגרות כהגנה).
...
- טון הדיבור: חומל, מגשר, מעצים ומעורר מחשבה, המדריך את ההורה ו/ואת הילד כיצד לעבור למצב של ויסות רגשי משותף (Co-regulation). שפה רגשית המתאימה לשיחות עומק אינטימיות ומשמעותיות בבית.
...
- טון הדיבור: מבין, מחבק ותומך בהורה, ומעניק לו כלים ליציבות ומניעת תסכול. דגש חזק על 'נוכחות הורית' (Parental Presence) וסמכות מבוססת ביטחון.
...
`;
    ...
    let prompt = `
אתה מאמן מנטלי ומטפל מומחה בעל הבנה פסיכולוגית עמוקה, המייעץ ומדריך מטעם בית הספר "לב המצפן". הגישה שלך היא: "ההתנהגות היא לא הבעיה. היא הרמז." ו-"אנשים מנסים לשנות התנהגות, כשמה שבאמת צריך להשתנות הוא הסיפור שהמוח מספר."

${coreTaskText}
${template ? `3. תבנית כתיבה להובלת הטקסט: "${template}"` : ""}
4. פלטפורמה וסגנון: ${platformPrompt}

${demographyInstructions}

הנחיות קריטיות ואסטרטגיות לכתיבה (חובה לפי הנחיות המערכת):
1. **טקסט נקי לפרומפטר (ללא הוראות בסוגריים!)**: אל תכלול בשום אופן הוראות בימוי, סוגריים, בסוגריים מרובעים, טון דיבור או מחוות (למשל אל תכתוב [מבט למצלמה] או [טון שקט]). הטקסט חייב להיות נקי 100%, קולח ומוכן לחלוטין לקריאה רציפה בפרומפטר!
2. **מנוע הוקים וניפוץ מיתוסים קליל עם עיגון מדעי/מוחי**: פתח בהוק חזק שמנפץ מיתוס נפוץ של הורים או מתבגרים. בגוף הסרטון, בגובה העיניים ובסגנון קליל, ענייני וחברי (כאילו אתה מסביר לחברה טובה בשיחת קפה, למשל: "אם את חושבת שכשאת קופצת תוך שנייה כשהבן שלך מבקש משהו את עוזרת לו - המדע מגלה ש..."), עגון את הדברים בעובדות על המוח המתבגר, פסיכולוגיה התפתחותית או מחקרים מוכחים ("מחקרים מראים ש...", "חקר המוח מוכיח ש..."). ללא כבדות אקדמית!
3. **הנעה אלגנטית לפעולה (בלי יומרנות ל"ריפוי בסרטון")**: אל תנסה "לרפא" או לשחרר את הבעיה בסרטון קצר. הסרטון מייצר הבנה ותובנה עמוקה ("Aha moment"). סיים בהכוונת הצופה בצורה אלגנטית ומכבדת לכך שהבנה היא רק הצעד הראשון, וכדי לעשות את השינוי האמיתי בבית צריכים תהליך ושיחת ייעוץ מקצועית ב"לב המצפן".
4. **רצף והמשכיות**: דאג שהסרטון יתחבר בטבעיות לסרטון הבא או להרחבה ביוטיוב (למשל: "אם הסרטון הזה הסביר איך הדפוס נוצר במוח, בסרטון הבא נראה איך זה נראה כשהוא טורק את הדלת...").
${includeSecondaryGain ? "5. **חשיפת הרווח המשני מול המחיר**: חובה לחשוף ולפרק באופן ישיר את 'הרווח המשני' של ההתנהגות/מנגנון (למשל: קבלת שליטה מדומה, הגנה או הימנעות) מול 'המחיר הכבד' שמשלמים עליה ברמת הקשר ובחוסן האישי." : ""}
`;
